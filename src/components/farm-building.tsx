...
export function FarmBuilding(props: FarmBuildingProps) {
  return (
    <div class="relative w-40 h-48 bg-slate-800 rounded-lg border border-slate-600 cursor-pointer hover:border-slate-400 transition-colors group overflow-visible mt-6">
      ...
      {/* Крыша с кнопкой */}
      <div class="absolute top-0 left-0 right-0 h-16 bg-green-900 rounded-t-lg">
        ...
        <div class="absolute -top-[52px] left-1/2 -translate-x-1/2 animate-shake">
          <button 
            ...
            class="relative w-12 h-12 bg-slate-900 rounded-full flex items-center justify-center hover:bg-green-500 hover:shadow-lg hover:shadow-green-500/50 transition-all focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 focus:ring-offset-slate-800"
          >
            <Icon path={wrench} class="w-7 h-7 text-green-400 group-hover:text-white" />
          </button>
      ...
      {/* Грядки */}
      <div class="absolute top-20 left-0 right-0 flex flex-col gap-2 px-2">
        <div class="flex justify-center space-x-1">
          <div class="w-8 h-4 bg-green-800 rounded-sm" />
          <div class="w-8 h-4 bg-green-800 rounded-sm" />
          <div class="w-8 h-4 bg-green-800 rounded-sm" />
        </div>
        <div class="flex justify-center space-x-1">
      ...
      {/* Основание */}
      <div class="absolute bottom-0 left-0 right-0 h-8 bg-green-900 rounded-b-lg" />

      {/* Текст */}
      <div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
        <div class="text-center">
          <div class="font-medium">Ферма</div>
        </div>
      </div>

      {/* Эффект свечения при наведении */}
      <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
        <div class="absolute inset-0 bg-green-400/5" />
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-green-400/10 rounded-full blur-xl" />
      </div>
    </div>
  );
}