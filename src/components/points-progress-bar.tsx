...
export const PointsProgressBar: Component<Props> = (props) => {
  const max = 120;
  const progress = createMemo(() => Math.min(props.points() / max * 100, 100));
  ...
  return (
    <div class="w-full relative flex items-center h-6 bg-black">
     <div 
      ...
      <div class="w-full flex justify-end items-center pr-1">{props.points()} <span class="pl-1 text-xs text-white/50">/ {max}</span></div>
    </div>
  );
}