...
export function ArmoryPage() {
  const [activeTab, setActiveTab] = createSignal<"weapons" | "materials">("weapons")
  ...
  const startWeaponCreation = () => {
    ...
    weaponTimer = setInterval(() => {
      setWeaponTimeLeft((prev) => {
        ...
        setWeaponProgress(((WEAPONS_CONFIG.productionTime - newTime) / WEAPONS_CONFIG.productionTime) * 100)
        return newTime
      })
    }, 1000)
  }
  ...
  const startProduction = () => {
    ...
    productionTimer = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        setProductionProgress(((MATERIALS_CONFIG.productionTime - newTime) / MATERIALS_CONFIG.productionTime) * 100)
        return newTime
      })
    }, 1000)
  }
  ...
  return (
    <BackLayout
      ...
      rightContent={
        <div class="px-2">
          <ResourceDisplay icon={MATERIALS_CONFIG.icon} value={accountState.materials} />
        </div>
      }
    >
      <div class="flex h-full flex-col">
        ...
        <div class="grid grid-cols-2 border-b border-slate-700/25">
          <button
            class={`p-4 text-sm ${
              activeTab() === "weapons" ? "border-b-2 border-blue-500 font-medium" : "text-slate-400"
            }`}
          ...
          <button
            class={`p-4 text-sm ${
              activeTab() === "materials" ? "border-b-2 border-blue-500 font-medium" : "text-slate-400"
            }`}
              ...
              <div class="flex flex-col h-full">
                <div class="flex-1 p-4">
                  ...
                  <div class="mt-4 text-center">
                    ...
                    <div class="mt-2 text-sm text-slate-400">
                      Требуется: {MATERIALS_CONFIG.icon} {WEAPONS_CONFIG.requiredMaterials}
                    </div>
                    {isCreatingWeapon() && (
                      <>
                        <div class="mt-4 px-4">
                        ...
                        <div class="mt-2 text-sm text-slate-400">{formatTime(weaponTimeLeft())}</div>
                      </>
                    )}
                    {!isCreatingWeapon() && <div class="mt-2 text-sm text-slate-400">3:00</div>}
                  </div>
                </div>
                <div class="flex-shrink-0 p-4 border-t border-slate-700/25">
                  {isCreatingWeapon() ? (
                    ...
                    >
                      Ускорить за {RESOURCES.DIAMOND.icon} {WEAPONS_CONFIG.speedUpCost}
                    </button>
                  ) : (
                    <button
                      onClick={weaponProgress() === 100 ? collectWeapon : startWeaponCreation}
            ...
            <Match when={activeTab() === "materials"}>
              <div class="flex flex-col h-full">
                <div class="flex-1 p-4">
                  ...
                  <div class="mt-4 text-center">
                    <div class="text-lg font-medium">{MATERIALS_CONFIG.name}</div>
                    {isProducing() && (
                      <>
                        <div class="mt-4 px-4">
                          <div class="h-2 w-full overflow-hidden rounded-full bg-slate-700">
                        ...
                        <div class="mt-2 text-sm text-slate-400">{formatTime(timeLeft())}</div>
                      </>
                    )}
                    {!isProducing() && <div class="mt-2 text-sm text-slate-400">6:00</div>}
                  </div>
                </div>
                <div class="flex-shrink-0 p-4 border-t border-slate-700/25">
                  {isProducing() ? (
                    <button
                      ...
                      class="w-full rounded-lg bg-purple-500 py-3 text-sm font-medium text-white hover:bg-purple-600"
                    >
                      Ускорить за {RESOURCES.DIAMOND.icon} {MATERIALS_CONFIG.speedUpCost}
                    </button>
                  ) : (
                    <button
                      onClick={productionProgress() === 100 ? collectProduction : startProduction}
                      class="w-full rounded-lg bg-blue-500 py-3 text-sm font-medium text-white hover:bg-blue-600"
                    >
