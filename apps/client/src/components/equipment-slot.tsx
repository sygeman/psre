      ...
      <div class="rounded-b bg-slate-900/80 py-0.5">
        <div class="flex justify-center">
          <div class="flex -space-x-0.5">
            {redStars > 0
              ? // Красные звезды
                Array.from({ length: redStars }).map(() => <StarIcon class="size-3 text-red-500" />)
              : // Желтые звезды
                Array.from({ length: yellowStars }).map(() => <StarIcon class="size-3 text-yellow-400" />)}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div class="flex flex-col items-center gap-2">
      ...
      >
        ...
        {props.canUpgrade && !props.isEmpty && <div class="absolute top-1 right-1 size-2 rounded-full bg-red-500" />}
