    ...
    <Show when={shouldShow()}>
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="p-8 text-center max-w-md mx-4">
          <h3 class="text-xl font-bold text-white mb-2">{getStatusInfo().title}</h3>

          <p class="text-slate-400 text-sm mb-6 leading-relaxed">{getStatusInfo().description}</p>
          ...
          <Show when={websocketStore.status === WebSocketStatus.Error}>
            <div class="mb-4">
              <div class="bg-red-900/30 border border-red-500/50 rounded-lg p-3 backdrop-blur-sm">
                <p class="text-red-300 text-xs font-mono break-words">{websocketStore.errorMessage}</p>
              </div>
            </div>
